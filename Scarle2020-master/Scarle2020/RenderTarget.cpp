...
RenderTarget::RenderTarget(ID3D11Device* _device, int _width, int _height) :m_width(_width), m_height(_height)
{
	...
	//set up blend state so it adds alphas together
...
void RenderTarget::SetShaderResourceView(ID3D11ShaderResourceView* shaderResource, ID3D11DeviceContext* deviceContext)
{
	deviceContext->CopyResource(m_renderTargetTexture,(ID3D11Texture2D*) shaderResource);
}
...
Color* RenderTarget::GetPixel(int x, int y, GameData* _GD)
{
	//grab a pixel within in the texture only when mapped
	if (m_mapped && m_byte)
	{	
		//Fixing upside down thing
		//y = (y*-1) + 160;
...
void RenderTarget::LoadTerrain(ID3D11Texture2D* tex, ID3D11DeviceContext* context)
{
	context->CopyResource(m_renderTargetTexture, tex);
}
